...
The site configuration is defined by the `config.yaml` file at the top level of the site directory. The file is in YAML format. The configuration has the following predefined values (check the [API documentation for `Nanoc::Site`](/docs/api/current/Nanoc/Site.html) for details about the default values):
...
To get the path of the compiled item, use [`Nanoc::Item#path`](/docs/api/current/Nanoc/Item.html#path-instance_method). This path is relative to the output directory; it starts with a slash which indicates the web root, i.e. the output directory. The index filenames are stripped off the end of the path. You can pass a `:rep` option to get the path of a specific representation. For example, the path of an item that is compiled to `output/foo/index.html` is `/foo/`.
...
An item’s list of representation can be fetched by calling [`Nanoc::Item#reps`](/docs/api//Nanoc/Item.html#reps-instance_method) instance. To get a specific rep, use [`Nanoc::Item#rep_named`](/docs/api/current/Nanoc/Item.html#rep_named-instance_method) with the rep name, like this:
...
To get the compiled content of a specific snapshot, use the `:snapshot` option of [`Nanoc::Item#compiled_content`](/docs/api/current/Nanoc/Item.html#compiled_content-instance_method) or [`Nanoc::ItemRep#compiled_content`](/docs/api/current/Nanoc/ItemRep.html#compiled_content-instance_method). For example:
...
To get an item attribute, use the [`Nanoc::Item#[]`](/docs/api/current/Nanoc/Item.html#%5B%5D-instance_method) method and pass the attribute key as a symbol, e.g. `@item[:author]`.
...
To use this helper, activate the [Rendering](/docs/api/current/Nanoc/Helpers/Rendering.html) helper (see the [helpers](#helpers) section for details), which is done by adding this line of code to some file in the `lib` directory (I recommend `lib/helpers.rb`):
...
The argument for the [`#route`](/docs/api/current/Nanoc/CompilerDSL.html#route-instance_method) method is the identifier of the item that should be compiled. It can also be a string that contains the `*` wildcard, which matches zero or more characters. Additionally, it can be a regular expression.

A `:rep` argument can be passed to the [`#route`](/docs/api/current/Nanoc/CompilerDSL.html#route-instance_method) call. This indicates the name of the representation this rule should apply to. This is `:default` by default, which means routing rules apply to the default representation unless specified otherwise.
...
nanoc comes with quite a handful of filters. [Here’s a list](http://nanoc.stoneship.org/docs/api/current/Nanoc/Filters.html).
...
Here’s a [list of helpers that come with nanoc](http://nanoc.stoneship.org/docs/api/current/Nanoc/Helpers.html).
...
New nanoc sites will have only one data source: the `filesystem_unified` one. For details about this data source, see the [documentation for `FilesystemUnified`](http://nanoc.stoneship.org/docs/api/current/Nanoc/DataSources/FilesystemUnified.html).
...
[`filesystem_unified`](/docs/api/current/Nanoc/DataSources/FilesystemUnified.html)
: Reads data from files on the disk. It is the default data source for new sites.

[`filesystem_verbose`](/docs/api/current/Nanoc/DataSources/FilesystemVerbose.html)
